# .cursor/rules.md
## 목적
- 이 저장소(toss-base)는 **Apps in Toss WebView 기반 미니앱**을 **React**로 빠르게 만들기 위한 템플릿이다.
- 사용자가 “아이디어/컨셉/기획 정보”를 주면, 그에 맞춰 **최소 기능(MVP) → 화면/흐름 → 구현 → 검증**까지 자동으로 진행한다.
- Apps in Toss의 제약/정책/SDK 사용은 반드시 공식 문서를 기준으로 한다. 필요 시 문서 근거를 먼저 요약한 뒤 구현한다. :contentReference[oaicite:1]{index=1}

---

## 0) 최상위 우선순위(절대 규칙)
1. **문서 우선(@docs 우선)**
    - Apps in Toss SDK, WebView 동작, 배포/빌드, 검수 관련 작업은 **구현 전에** 근거를 문서에서 확인하고 요약한다. :contentReference[oaicite:2]{index=2}
2. **TDS 필수 준수(WebView 비게임 앱)**
    - WebView 기반 비게임 미니앱은 **TDS 사용이 필수**이며, 검수 승인 기준에 포함된다. :contentReference[oaicite:3]{index=3}
3. **빌드 산출물 경로 일치**
    - `vite build` 결과물 경로(`vite.config.ts`의 `build.outDir`)는 `apps-in-toss.config.ts`의 `webBundleDir`와 **반드시 동일**해야 한다(불일치 시 배포 실패 가능). :contentReference[oaicite:4]{index=4}
4. **작게, 빠르게, 검증 가능하게**
    - 한 번에 큰 변경 금지. 기능은 “패킷(30~60분)” 단위로 쪼개고, 각 패킷마다 완료조건(DoD)을 먼저 정의한 뒤 구현한다.

---

## 1) 기본 기술 스택 규칙(WebView + React)
- 프론트는 **React + TypeScript** 기준으로 작성한다.
- 라우팅/상태/에러/로딩/빈상태는 초기부터 포함한다(나중에 붙이지 않는다).
- 외부 라이브러리 추가는 최소화한다. 추가가 필요하면:
    - 왜 필요한지(대체 불가 이유),
    - 번들/런타임 영향,
    - 제거 가능성(추후 제거 계획)
      를 함께 제시한다.
- UI는 **TDS 컴포넌트 우선**으로 구성한다. 커스텀 UI는 TDS로 불가능한 경우에만 제한적으로 허용한다. :contentReference[oaicite:5]{index=5}
- **디자인(Toss Look)**: UI/스타일 작업 시 `desigin/toss-look.md`와 `src/design/tokens.ts`를 따른다. 여백·모서리·타이포는 토큰만 사용(임의 px 하드코딩 금지). 섹션은 "제목 → 내용 → 액션" 순서, CTA 1개 명확.

---

## 2) “아이디어 → 자동 생성” 표준 절차(반드시 이 순서)
사용자가 앱 아이디어/컨셉을 주면, 아래 5단계를 고정으로 수행한다.

### Step A. 1페이지 기획 요약(자동)
- 목표(사용자/문제/가치)
- 핵심 기능 3개(필수) + 선택 기능 3개(후순위)
- 수익/전환 포인트(있다면)
- 리스크/제약(앱인토스 WebView 관점 포함)

### Step B. 화면/흐름 설계(자동)
- 정보구조(IA)와 라우트 목록
- 각 화면의 상태: Loading / Empty / Error / Success
- 이벤트/트래킹 포인트(필요 시)

### Step C. 구현 계획(패킷화, 자동)
- 30~60분 단위 패킷으로 작업을 분해
- 각 패킷의 DoD(완료조건) 5개씩 명시
- 의존성(필요 문서/SDK/설정) 나열

### Step D. 코드 생성/수정(자동)
- DoD를 만족하는 최소 변경만 수행
- 문서/제약이 걸린 부분은 근거를 먼저 제시하고 진행 :contentReference[oaicite:6]{index=6}
- 빌드 산출물 경로(webBundleDir) 규칙을 깨지 않도록 설정/스크립트 유지 :contentReference[oaicite:7]{index=7}

### Step E. 결과물 보고(자동, 매번 출력)
- 변경 요약(3줄)
- DoD 체크리스트(체크박스)
- 변경 파일 목록
- 검증 방법(로컬 실행/빌드 확인 포함)
- 리스크/후속 작업 2개

---

## 3) 파일/설정 관련 규칙(Apps in Toss 빌드 포함)
- `apps-in-toss.config.ts`의 `webBundleDir`은 배포 파이프라인의 기준이다.
    - 빌드 결과물이 다른 폴더로 떨어지지 않도록 스크립트를 맞춘다. :contentReference[oaicite:8]{index=8}
- 빌드/배포 관련 파일 수정 시 반드시:
    - 변경 전/후 동작 차이
    - webBundleDir ↔ vite build.outDir 일치 여부
    - 로컬 build 결과 확인 방법
      을 함께 기록한다. :contentReference[oaicite:9]{index=9}

---

## 4) 코드 품질 규칙(실행 안정성)
- 네트워크/비동기 로직이 있는 모든 화면은:
    - 로딩 상태,
    - 에러 상태(재시도 포함),
    - 빈 상태,
    - 성공 상태
      를 반드시 구현한다.
- 사용자 입력은:
    - 검증(Validation),
    - 중복 제출 방지,
    - 실패 메시지
      를 포함한다.
- 콘솔 로그 남발 금지. 필요 시 최소한의 진단 로그만 사용한다.

---

## 5) 커뮤니케이션 규칙(요청/응답 형식)
- 모호한 요구가 오면 질문을 남발하지 않는다. 대신:
    1) 가능한 합리적 가정 2~3개를 제시하고,
    2) 그 중 기본값을 선택해 진행하며,
    3) 선택한 이유를 2줄로 설명한다.
- 답변은 “결론 → 근거(문서/제약) → 변경사항 → 검증방법” 순서로 짧고 명확하게 작성한다.

---

## 6) 금지 사항
- Apps in Toss WebView/TDS/배포 경로 규칙을 무시한 구현 금지 :contentReference[oaicite:10]{index=10}
- 큰 리팩토링을 기능 작업과 동시에 진행 금지(패킷 분리)
- 근거 없는 “추정 구현” 금지(문서/샘플 확인 후 진행)
